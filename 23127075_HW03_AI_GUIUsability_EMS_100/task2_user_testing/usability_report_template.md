...
| --- | --- | --- | --- | --- | ---: | --- | --- | --- |
| UT-D-001 | D3 | Usability | Admin vẫn nhìn thấy/đi vào các lựa chọn support request mang tính user, làm participant lẫn giữa danh sách request cá nhân và khu vực xử lý request của admin. | P02, P04, P05 đều ghi nhận nhầm luồng admin/user. | 2 | Tách rõ menu admin và user; đổi label admin thành `Admin Support Requests`; ẩn hoặc phân nhóm các mục user khi đang ở admin context. | `session_notes/image/P02_session_notes/1785855764060.png`; `session_notes/image/P04_session_notes/1785915787357.png`; `session_notes/image/P05_session_notes/1785938625303.png` | No |
| UT-D-002 | D3 | Usability | `Pending`/`Resolved` chưa đủ affordance như tab/filter, khiến participant không nhận ra ngay có thể chuyển trạng thái hoặc hiểu nhầm là card tổng quan. | P02 không để ý có 2 tab; P03 không nhận ra tab switch; P05 nói filter status dễ gây nhầm. | 2 | Thiết kế lại thành segmented control/tab rõ ràng, đặt gần nhóm filter và hiển thị active state mạnh hơn. | `session_notes/image/P02_session_notes/1785855715511.png`; `session_notes/image/P03_session_notes/1785859136741.png`; `session_notes/image/P05_session_notes/1785938635976.png` | No |
| UT-D-003 | D3 | Usability | Request description chứa chuỗi dài không tự xuống dòng, bị tràn ngang khỏi khung nội dung và giảm khả năng đọc khi xử lý request. | P01 ghi nhận nội dung request description bị tràn ra ngoài. | 2 | Áp dụng `word-break`/`overflow-wrap`, giới hạn chiều rộng cột và cho phép xem đầy đủ trong detail/modal. | `session_notes/image/P01_session_notes/1785821851780.png` | No |
| UT-D-004 | D1 | Usability | Khi click vào field của form tạo support request, caret/focus chưa đủ rõ, làm participant không chắc field đã sẵn sàng nhập. | P04 ghi nhận con trỏ không nhấp nháy rõ từ đầu; P02 cũng nhắc màu field làm khó thấy dấu nháy. | 1 | Tăng contrast focus ring/caret, dùng border rõ hơn khi input active. | `session_notes/image/P04_session_notes/1785915794108.png` | No |
| UT-D-005 | D3 | Usability | Bố cục bảng admin dành nhiều chiều rộng cho cột `REQUEST`, trong khi date range/filter chưa được nhóm trực quan, làm giảm khả năng quét và lọc request. | P03 nhận xét cột `REQUEST` khá rộng và filter date range nên để một hàng mới. | 1 | Cân lại width các cột, gom filter theo hàng/cụm rõ ràng và giữ nhãn filter gần control. | `session_notes/image/P03_session_notes/1785859116174.png` | No |
| UT-D-006 | D2/D3 | Usability | Control `Rows per page` xuất hiện nhưng participant ghi nhận số lượng dòng mỗi trang bị cố định ở cả user/admin, làm giảm cảm giác kiểm soát khi danh sách dài. | P03 ghi nhận rows per page bị fixed. | 1 | Nếu không cho đổi số dòng thì ẩn control; nếu có control thì cho chọn các giá trị hợp lệ. | `session_notes/image/P03_session_notes/1785858880325.png` | No |
| UT-D-007 | D2/D3 | Usability | Giao diện user/admin chưa đồng nhất về màu sắc và sidebar, khiến hai khu vực có cảm giác thuộc hai hệ giao diện khác nhau. | P05 ghi nhận sidebar user sáng, sidebar admin tối và layout user/admin không đồng nhất. | 1 | Chuẩn hóa design token/layout giữa dashboard user và admin, chỉ dùng khác biệt màu sắc để thể hiện role khi có chủ đích. | `session_notes/image/P05_session_notes/1785939135603.png`; `session_notes/image/P05_session_notes/1785939155583.png` | No |
...
| --- | --- | --- |
| UT-E-001 | `session_notes/P01_session_notes.md` | Raw notes P01, SUS `82.5`, evidence description overflow. |
| UT-E-002 | `session_notes/P02_session_notes.md` | Raw notes P02, SUS `85.0`, evidence admin/user navigation và status tabs. |
| UT-E-003 | `session_notes/P03_session_notes.md` | Raw notes P03, SUS `75.0`, evidence rows per page, admin table/filter và status tabs. |
| UT-E-004 | `session_notes/P04_session_notes.md` | Raw notes P04, SUS `82.5`, evidence admin/user navigation, focus/caret và label inconsistency. |
| UT-E-005 | `session_notes/P05_session_notes.md` | Raw notes P05, SUS `50.0`, evidence admin/user navigation, status filter và visual inconsistency. |
| UT-E-006 | `session_notes/image/P01_session_notes/1785821851780.png` | Description dài bị tràn trong D3. |
| UT-E-007 | `session_notes/image/P02_session_notes/1785855764060.png` | Participant P02 lẫn menu/request user và admin. |
| UT-E-008 | `session_notes/image/P02_session_notes/1785855715511.png` | P02 không để ý hai tab trạng thái. |
| UT-E-009 | `session_notes/image/P03_session_notes/1785858880325.png` | `Rows per page` ở list request. |
| UT-E-010 | `session_notes/image/P03_session_notes/1785859116174.png` | Bố cục cột `REQUEST` và filter admin. |
| UT-E-011 | `session_notes/image/P03_session_notes/1785859136741.png` | `Pending`/`Resolved` chưa giống tab rõ ràng. |
| UT-E-012 | `session_notes/image/P04_session_notes/1785915787357.png` | P04 lẫn support request user/admin. |
| UT-E-013 | `session_notes/image/P04_session_notes/1785915794108.png` | Focus/caret field form chưa rõ. |
| UT-E-014 | `session_notes/image/P04_session_notes/1785915761757.png` | Label `My Support Requests` trong admin gây không nhất quán. |
| UT-E-015 | `session_notes/image/P05_session_notes/1785938625303.png` | P05 lẫn danh sách request user và admin. |
| UT-E-016 | `session_notes/image/P05_session_notes/1785938635976.png` | Status filter D3 dễ bị hiểu như card tổng quan. |
| UT-E-017 | `session_notes/image/P05_session_notes/1785939135603.png` | Dashboard user với sidebar/filter nền sáng. |
| UT-E-018 | `session_notes/image/P05_session_notes/1785939155583.png` | Dashboard admin với sidebar tối, tạo cảm giác thiếu đồng nhất. |
