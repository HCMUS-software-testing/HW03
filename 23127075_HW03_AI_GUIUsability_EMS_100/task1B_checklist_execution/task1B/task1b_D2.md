...
```text
@Superpowers Đọc các file trong folder docs để hiểu bối cảnh bài tập. Thực hiện Task 1B cho Kiên trên Scene D2. Dùng ../../task1A_group_checklist/group/gui_usability_checklist_final.md (không thay đổi file này) để làm check list. Tạo 1 file markdown cho task1b_D2.md trong folder submission/task1B cho việc testing dựa trên checklist đó. Hãy tận dụng MCP Playwright và Chrome Devtools trong quá trình test. Thông tin link web, tài khoản admin và user đã được cung cấp trong test access notes used during execution. Tuyệt đối tuân theo rule được ghi trong  test access notes used during execution. Nếu cần thêm gì thì hãy yêu cầu tôi cung cấp. Format cho output của D2 cũng nên tương tự như D1.
```
...
| --- | --- | --- |
| Đọc bối cảnh HW03 | Shell `rtk` | Đã đọc tài liệu trong `docs/`, `test access notes used during execution`, checklist final và file D1 để giữ đúng phạm vi Scenario D và format báo cáo. |
...
| --- | --- | --- |
| E-D2-001 | `../screenshots/D2/D2_my_requests_overview.png` | Tổng quan D2 list ở desktop với 2 resolved requests của Kiên. |
| E-D2-002 | `../screenshots/D2/D2_detail_with_response.png` | Detail request `/complaints/56` có status, nội dung, attachment và `Support response`. |
| E-D2-003 | `../screenshots/D2/D2_attachment_lightbox.png` | Attachment mở trong lightbox/modal. |
| E-D2-004 | `../screenshots/D2/D2_search_status_filter.png` | Search `keyboard` kết hợp filter `Resolved` trả 1 kết quả. |
| E-D2-005 | `../screenshots/D2/D2_mobile_detail_response.png` | Detail và response ở viewport mobile `390x844`. |
| F-D2-001 | `../screenshots/checklist-failures/F-D2-001_back-loses-filter-context.png` | `Back` từ detail làm mất search/status filter. |
| F-D2-002 | `../screenshots/checklist-failures/F-D2-002_search-no-results-misleading-empty-state.png` | Empty state khi search không có kết quả dùng message `No requests yet` và không có reset action rõ. |
| F-D2-003 | `../screenshots/checklist-failures/F-D2-003_mobile-floating-button-near-pagination.png` | Floating social links button chồng sát/che vùng pagination trên mobile list. |
